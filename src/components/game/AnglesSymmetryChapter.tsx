...
export default function AnglesSymmetryChapter({ onComplete, childName }: Props) {
  ...
  const [round, setRound] = useState<Round>(() => makeDistinct(() => makeRound(1), seen.current, r => `${r.type}:${r.deg ?? r.kind}`))
  ...
  useEffect(() => {
    ...
    const r = makeDistinct(() => makeRound(ada.difficulty), seen.current, r => `${r.type}:${r.deg ?? r.kind}`)

...
export default function AnglesSymmetryChapter({ onComplete, childName }: Props) {
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.visual}>
        {round.type === 'angle' ? <AngleView deg={round.deg!} /> : <SymShape kind={round.kind!} show={false} />}
      </div>
      ...
      {reMed?.phase === 'check' && (
        <ReMedOverlay>
          <ASPick prompt={reMed.round.question} options={reMed.round.choices} answer={reMed.round.answer}
            visual={reMed.round.type === 'angle' ? <AngleView deg={reMed.round.deg!} /> : <SymShape kind={reMed.round.kind!} show={false} />}

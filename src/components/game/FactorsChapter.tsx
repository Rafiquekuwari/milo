...
export default function FactorsChapter({ onComplete, childName }: Props) {
  ...
  const roundSig = (r: Round) => `${r.type}|${r.n}|${r.base ?? ''}|${r.answer}`
  const [round, setRound] = useState<Round>(() => makeDistinct(() => makeRound(1), seen.current, roundSig))
  ...
  useEffect(() => {
    ...
    const r = makeDistinct(() => makeRound(ada.difficulty), seen.current, roundSig)

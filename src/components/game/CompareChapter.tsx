...
export default function CompareChapter({ onComplete, childName }: Props) {
  ...
  const seen = useRef<Set<string>>(new Set())   // (a,b) pairs already asked this session
  const [roundIdx, setRoundIdx] = useState(0)
  const [round, setRound] = useState<Round>(() => makeDistinct(() => makeRound(1), seen.current, r => `${r.a},${r.b}`))
  ...
  function loadRound(idx: number) {
    const r = makeDistinct(() => makeRound(ada.difficulty), seen.current, r => `${r.a},${r.b}`)

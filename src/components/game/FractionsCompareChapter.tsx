...
export default function FractionsCompareChapter({ onComplete, childName }: Props) {
  ...
  const [round, setRound] = useState<Round>(() => makeDistinct(() => makeRound(1), seen.current, r => `${r.type}|${r.question}|${r.answer}`))
  ...
  useEffect(() => {
    ...
    const r = makeDistinct(() => makeRound(ada.difficulty), seen.current, r => `${r.type}|${r.question}|${r.answer}`)

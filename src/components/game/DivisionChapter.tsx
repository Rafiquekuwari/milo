'use client'
/**
 * DivisionChapter (9–11) — division by sharing, facts, and remainders.
 * Adaptive: L1 exact facts (no remainder), L2 facts to ÷10 + small remainders,
 * L3 bigger remainders. 10 rounds, after-3-wrong re-teach (share into bins, then
...
import DivisionLesson, { ShareWatch, DivPick } from '../lessons/DivisionLesson'
...
interface Round { total: number; groups: number; rem: number; answer: string; choices: string[]; say: string }
...
function makeRound(d: 1 | 2 | 3): Round {
  const exact = (groups: number, q: number): Round => {
    const total = groups * q
    return { total, groups, rem: 0, answer: String(q), choices: noRemChoices(q), say: `Share ${total} among ${groups}. How many each?` }
  }
  const withRem = (groups: number, q: number, rem: number): Round => {
    const total = groups * q + rem
    return { total, groups, rem, answer: `${q} r ${rem}`, choices: remChoices(q, rem, groups), say: `Share ${total} among ${groups}. How many each, and how many left over?` }
  }
  if (d === 1) return exact(rint(2, 5), rint(2, 5))
  if (d === 2) {
    if (Math.random() < 0.5) return exact(rint(2, 9), rint(2, 9))
    const groups = rint(3, 5)
    return withRem(groups, rint(2, 5), rint(1, groups - 1))
  }
  const groups = rint(3, 9)
  return withRem(groups, rint(3, 8), rint(1, groups - 1))
}

type ReMed = { phase: 'reteach' | 'check'; round: Round } | null

export default function DivisionChapter({ onComplete, childName }: Props) {
  const { phase, startPractice } = useChapterPhase()
  const { speak } = useMiloSpeaker()
  const ada = useAdaptive('division')
  const seen = useRef<Set<string>>(new Set())   // question signatures asked this session
  const [roundIdx, setRoundIdx] = useState(0)
  const [round, setRound] = useState<Round>(() => makeDistinct(() => makeRound(1), seen.current, r => `${r.total}/${r.groups}`))
  ...
  useEffect(() => {
    ...
    const r = makeDistinct(() => makeRound(ada.difficulty), seen.current, r => `${r.total}/${r.groups}`)
    setRound(r); setSelected(null); setFeedback(null)
    speakAfterCurrent(roundIdx === 0 ? `Hi ${childName}! ${r.say}` : r.say)
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [roundIdx, phase])

  if (phase === 'lesson') return <DivisionLesson childName={childName} onLessonComplete={startPractice} />
  ...
  const bubbleText = selected !== null
    ...
    : round.rem > 0 ? 'How many each, and the remainder?' : 'How many in each group?'

  return (
    <div style={S.page}>
      <SpeakingLock />
      <GameTopbar chapterName="Division" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} />
      ...
      <div style={S.topRow}>
        ...
        {ada.shouldHint && <span style={S.hintTag}>💡 Share one to each!</span>}
      </div>
      <div style={S.miloRow}>
      ...
      <div style={S.visual}>
        <div style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 56, lineHeight: 1, color: 'var(--milo-orange)', textShadow: '0 4px 0 rgba(61,37,22,.12)' }}>{round.total} ÷ {round.groups}</div>
      </div>
      ...
      {reMed?.phase === 'reteach' && (
        <ReMedOverlay>
          <ShareWatch total={reMed.round.total} groups={reMed.round.groups}
            intro={`Let's share ${reMed.round.total} into ${reMed.round.groups} together.`} outro="Now you try!"
            onDone={() => setReMed({ ...reMed, phase: 'check' })} />
        </ReMedOverlay>
      )}
      {reMed?.phase === 'check' && (
        <ReMedOverlay>
          <DivPick total={reMed.round.total} groups={reMed.round.groups} choices={reMed.round.choices}
            intro="Now you pick the answer!" outro={`Yes! ${reMed.round.answer}!`} onDone={finishReMed} />
        </ReMedOverlay>
      )}
    </div>
  )

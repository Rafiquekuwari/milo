...
export default function MultiplicationChapter({ onComplete, childName }: Props) {
  ...
  const [round, setRound] = useState<Round>(() => makeDistinct(() => makeRound(1), seen.current, r => `${r.mode}:${r.g}x${r.per}`))
  ...
  function loadRound(idx: number) {
    const r = makeDistinct(() => makeRound(ada.difficulty), seen.current, r => `${r.mode}:${r.g}x${r.per}`)
  ...
  function handleAnswer(choice: number) {
    ...
    else { setWrong(w => w + 1); speakAt(`Almost! ${round.g} times ${round.per} is ${round.answer}. ${ada.encouragement}`, answerRef.current) }
    afterSpeech(() => {
      setFeedback(null)

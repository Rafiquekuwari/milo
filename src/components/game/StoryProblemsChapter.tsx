...
export default function StoryProblemsChapter({ onComplete, childName }: Props) {
  ...
  function handleAnswer(choice: number) {
    ...
    else { setWrong(w => w + 1); speakAt(`The answer is ${problem.answer}. ${ada.encouragement}`, answerRef.current) }
    afterSpeech(() => {
      setFeedback(null)
    ...
    <div style={S.page}>
      ...
      <div style={S.miloRow}>
        ...
        <div className="milo-bubble" style={{ flex: 1, fontSize: 19 }}>
          {selected !== null ? (feedback === 'correct' ? '🎉 Correct!' : `The answer is ${problem.answer}.`) : problem.story}
        </div>
      </div>
      ...
      {feedback && <div style={{ ...S.flash, background: feedback === 'correct' ? 'var(--garden-green)' : 'var(--apple-red)' }}>
        {feedback === 'correct' ? `✅ ${problem.answer}!` : `It was ${problem.answer}`}
      </div>}

'use client'
/**
 * StoryProblemsChapter (6–8) — dynamic word problems. Each round = template +
 * theme + random numbers → a spoken story; the child picks the answer. Adaptive:
 *   L1 add/take-away within 10 (pictures)
 *   L2 add/take-away within 20 (pictures)
 *   L3 + "how many more" (compare), two-digit numbers (no pictures)
 * 10 rounds, after-3-wrong re-teach (act the story out, then a check).
 */
...
import StoryProblemsLesson, {
  THEMES, NAMES, makeProblem, StoryScene, StoryWatch, StoryAsk,
...
function makeStory(d: 1 | 2 | 3): Problem {
  const types: StoryType[] = d === 3 ? ['add', 'sub', 'compare'] : ['add', 'sub']
  const type = pick(types)
  const theme = pick(THEMES)
  const name = pick(NAMES)
  let friend = pick(NAMES); while (friend === name) friend = pick(NAMES)

  let a: number, b: number
  if (type === 'add') {
    if (d === 1) { a = rint(1, 8); b = rint(1, 10 - a) }
    else if (d === 2) { a = rint(2, 15); b = rint(1, 20 - a) }
    else { a = rint(11, 70); b = rint(5, Math.min(29, 99 - a)) }
  } else if (type === 'sub') {
    if (d === 1) { a = rint(3, 10); b = rint(1, a - 1) }
    else if (d === 2) { a = rint(6, 20); b = rint(1, a - 1) }
    else { a = rint(20, 99); b = rint(2, a - 1) }
  } else { // compare (L3): name has the bigger amount
    a = rint(20, 99); b = rint(1, a - 1)
  }
  return makeProblem(type, a, b, theme, name, friend, d < 3)
}

type ReMed = { phase: 'reteach' | 'check'; problem: Problem } | null

export default function StoryProblemsChapter({ onComplete, childName }: Props) {
  ...
  function handleAnswer(choice: number) {
    ...
    else { setWrong(w => w + 1); speakAt(`It's ${problem.answer}. ${ada.encouragement}`, answerRef.current) }
    afterSpeech(() => {
      setFeedback(null)
  ...
  if (phase === 'lesson') return <StoryProblemsLesson childName={childName} onLessonComplete={startPractice} />

  return (
    <div style={S.page}>
      <SpeakingLock />
      <GameTopbar chapterName="Story Problems" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} />
      ...
      <div style={S.topRow}>
        ...
        {ada.shouldHint && <span style={S.hintTag}>💡 Add or take away?</span>}
      </div>
      <div style={S.miloRow}>
        <img src="/assets/characters/milo-happy.png" alt="Milo" style={S.milo}
          onError={e => { (e.target as HTMLImageElement).style.display = 'none' }} />
        <div className="milo-bubble" style={{ flex: 1, fontSize: 19 }}>
          {selected !== null ? (feedback === 'correct' ? '🎉 Correct!' : `It's ${problem.answer} — now you know.`) : problem.story}
        </div>
      </div>

      {problem.showScene && <div style={S.sceneBox}><StoryScene problem={problem} /></div>}

      <div style={S.qPill}>{problem.question}</div>

      <button onClick={() => speak(problem.say)} style={S.replayWrap}><span style={S.replay}>🔊 Read again</span></button>

      <div style={S.choicesRow}>
        {problem.choices.map(ch => {
          const isSel = selected === ch, isOk = ch === problem.answer
          return (
      ...
      {feedback && <div style={{ ...S.flash, background: feedback === 'correct' ? 'var(--garden-green)' : 'var(--milo-orange)' }}>
        {feedback === 'correct' ? `✅ ${problem.answer}!` : `It's ${problem.answer} — now you know! 🙂`}
      </div>}
      <p style={S.roundLabel}>Round {Math.min(roundIdx + 1, TOTAL_ROUNDS)} of {TOTAL_ROUNDS}</p>

      {reMed?.phase === 'reteach' && (
        <ReMedOverlay>
          <StoryWatch problem={reMed.problem} onDone={() => setReMed({ ...reMed, phase: 'check' })} />
        </ReMedOverlay>
      )}
      {reMed?.phase === 'check' && (
        <ReMedOverlay>
          <StoryAsk problem={reMed.problem} outro={`Yes! The answer is ${reMed.problem.answer}!`} onDone={finishReMed} />
        </ReMedOverlay>
      )}
    </div>
  )
...
const S: Record<string, React.CSSProperties> = {
  page: { minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '72px 24px 32px', gap: 14, position: 'relative', background: 'linear-gradient(180deg,var(--sun-yellow-soft) 0%,var(--bg-page) 55%)' },
  ...
  miloRow: { display: 'flex', alignItems: 'flex-end', gap: 14, width: '100%', maxWidth: 560 },
  milo: { width: 84, height: 84, objectFit: 'contain', flexShrink: 0, filter: 'drop-shadow(0 4px 10px rgba(61,37,22,.15))' },
  sceneBox: { display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,.65)', border: '4px solid var(--outline)', borderRadius: 24, padding: '16px 20px', boxShadow: '0 6px 0 rgba(61,37,22,.08)', maxWidth: 620, width: '100%', minHeight: 90 },

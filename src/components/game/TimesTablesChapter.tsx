'use client'
/**
 * TimesTablesChapter (9–11) — multiplication fluency and 2-digit × 1-digit.
 * Adaptive: L1 facts to 5×, L2 facts to 9× + teen × single, L3 full 2-digit ×
 * 1-digit. 10 rounds, after-3-wrong re-teach (array skip-count or place-value
 * split, then check). See docs/curriculum-9-11.md.
 */
...
import TimesTablesLesson, { FactWatch, SplitWatch, MultPick } from '../lessons/TimesTablesLesson'
...
interface Round { type: QType; a: number; b: number; question: string; say: string; answer: number; choices: number[] }
...
function makeRound(d: 1 | 2 | 3): Round {
  ...
  if (t === 'fact') {
    ...
    return { type: t, a, b, question: `${a} × ${b}`, say: `What is ${a} times ${b}?`, answer: a * b, choices: multChoices(a * b, a, b) }
  }
  const n = d === 2 ? rint(11, 19) : rint(11, 99)
  const k = d === 2 ? rint(2, 4) : rint(2, 9)
  return { type: t, a: n, b: k, question: `${n} × ${k}`, say: `What is ${n} times ${k}?`, answer: n * k, choices: multChoices(n * k, n, k) }
}

type ReMed = { phase: 'reteach' | 'check'; round: Round } | null

export default function TimesTablesChapter({ onComplete, childName }: Props) {
  const { phase, startPractice } = useChapterPhase()
  const { speak } = useMiloSpeaker()
  const ada = useAdaptive('timesTables')
  ...
  if (phase === 'lesson') return <TimesTablesLesson childName={childName} onLessonComplete={startPractice} />
  ...
  function handleAnswer(choice: number) {
    ...
    else { setWrong(w => w + 1); speakAt(`${round.a} times ${round.b} is ${round.answer}. ${ada.encouragement}`, answerRef.current) }
    afterSpeech(() => {
      setFeedback(null)
  ...
  return (
    <div style={S.page}>
      <SpeakingLock />
      <GameTopbar chapterName="Times Tables" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} />
      ...
      <div style={S.topRow}>
        ...
        {ada.shouldHint && <span style={S.hintTag}>💡 {round.type === 'twoDigit' ? 'Split tens & ones!' : 'Skip-count!'}</span>}
      </div>
      <div style={S.miloRow}>
      ...
      <div style={S.visual}>
        <div style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 56, lineHeight: 1, color: 'var(--milo-orange)', textShadow: '0 4px 0 rgba(61,37,22,.12)' }}>{round.question}</div>
      </div>
      ...
      {reMed?.phase === 'reteach' && (
        <ReMedOverlay>
          {reMed.round.type === 'twoDigit'
            ? <SplitWatch n={reMed.round.a} k={reMed.round.b} intro={`Let's split ${reMed.round.a} times ${reMed.round.b} into tens and ones.`} outro="Now you try!" onDone={() => setReMed({ ...reMed, phase: 'check' })} />
            : <FactWatch a={reMed.round.a} b={reMed.round.b} intro={`Let's count ${reMed.round.a} rows of ${reMed.round.b}.`} outro="Now you try!" onDone={() => setReMed({ ...reMed, phase: 'check' })} />}
        </ReMedOverlay>
      )}
      {reMed?.phase === 'check' && (
        <ReMedOverlay>
          <MultPick a={reMed.round.a} b={reMed.round.b} choices={reMed.round.choices} intro="Now you pick the product!" outro={`Yes! ${reMed.round.answer}!`} onDone={finishReMed} />
        </ReMedOverlay>
      )}
    </div>
  )

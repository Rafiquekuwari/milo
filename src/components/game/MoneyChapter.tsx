'use client'
/**
 * MoneyChapter (6–8) — count a handful of generic coins (1, 5, 10, 25). Adaptive:
 * L1 a few 1s & 5s → L2 adds 10s → L3 adds 25s and more coins. 10 rounds,
 * after-3-wrong re-teach (count the coins up, then a check).
 */
...
import MoneyLesson, { CoinRow, CoinWatch, CoinAsk, buildCoinChoices } from '../lessons/MoneyLesson'
...
interface Round { coins: number[]; answer: number; choices: number[]; say: string }

function makeRound(d: 1 | 2 | 3): Round {
  const pool = d === 1 ? [1, 5] : d === 2 ? [1, 5, 10] : [1, 5, 10, 25]
  const n = d === 1 ? rint(2, 3) : d === 2 ? rint(3, 4) : rint(4, 5)
  const coins = Array.from({ length: n }, () => pick(pool)).sort((a, b) => b - a)
  const answer = coins.reduce((s, v) => s + v, 0)
  return { coins, answer, choices: buildCoinChoices(answer), say: 'How much money?' }
}

type ReMed = { phase: 'reteach' | 'check'; round: Round } | null

export default function MoneyChapter({ onComplete, childName }: Props) {
  const { phase, startPractice } = useChapterPhase()
  const { speak } = useMiloSpeaker()
  const ada = useAdaptive('money')
  ...
  function loadRound(idx: number) {
    ...
    speakAfterCurrent(idx === 0 ? `Hi ${childName}! How much money?` : r.say)
  }
  ...
  return (
    <div style={S.page}>
      <SpeakingLock />
      <GameTopbar chapterName="Money" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} />
      ...
      <div style={S.topRow}>
        ...
        {ada.shouldHint && <span style={S.hintTag}>💡 Add from the biggest!</span>}
      </div>
      <div style={S.miloRow}>
      ...
      <div style={S.sceneBox}><CoinRow coins={round.coins} /></div>
      ...
      {reMed?.phase === 'reteach' && (
        <ReMedOverlay>
          <CoinWatch coins={reMed.round.coins} intro="Let's count the coins together." onDone={() => setReMed({ ...reMed, phase: 'check' })} />
        </ReMedOverlay>
      )}
      {reMed?.phase === 'check' && (
        <ReMedOverlay>
          <CoinAsk coins={reMed.round.coins} choices={reMed.round.choices} intro="Now you count! How much money?" outro={`Yes! ${reMed.round.answer}!`} onDone={finishReMed} />
        </ReMedOverlay>
      )}
    </div>
  )
...
const S: Record<string, React.CSSProperties> = {
  page: { minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '72px 24px 32px', gap: 16, position: 'relative', background: 'linear-gradient(180deg,var(--sun-yellow-soft) 0%,var(--bg-page) 55%)' },
  ...
  sceneBox: { display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,.65)', border: '4px solid var(--outline)', borderRadius: 24, padding: '20px 22px', boxShadow: '0 6px 0 rgba(61,37,22,.08)', maxWidth: 620, width: '100%', minHeight: 96 },

...
9. **Dead code from the Ch.3 swap:** `src/components/lessons/BiggerSmallerLesson.tsx` (exports `BiggerSmallerLesson` + `CompareCards` + `CSS`) is now orphaned — the old `NumberComparisonChapter` drill was its only consumer. Safe to delete in a cleanup pass (same as Ch.2's `order.tsx` removal).
